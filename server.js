...
const server = app.listen(PORT, async () => {
    ...
    try {
        ...
        console.error('👉 Vérifie que XAMPP/MAMP est lancé et que la BDD "app_db" existe.');
    }
});

server.on('error', (err) => {
    if (err.code === 'EADDRINUSE') {
        console.error(`❌ Le port ${PORT} est déjà utilisé. Essaie de changer le port dans le fichier .env.`);
    } else {
